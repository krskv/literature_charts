(function( $ ) {
	...
	$('.diagram').on('mouseenter', '.with_info_box', function() {
	...
	$('.diagram').on('mouseleave', '.with_info_box', function() {
	...
	$.fn.percentLine = function(line_data,color_theme) {
		...
		line_data.map( (obj ,index) => {
			$(this).append('<div data-percent="' + obj.value.toFixed(1).replace(',','.') + '%" data-title="' + obj.title + '" class="percent_line_sector with_info_box" style="width: ' + obj.value + '% ; background:' + colors[index] + '"></div>');
		});
	}
	...
	$.fn.ballsDiagram = function(data,color_theme) {
		...
		data.map( (obj,index) => {
			...
			var ball_padding = (( 15 - ball_size ) / 2).toFixed(2);

			$(this).append('<div class="balls_diagram__row"><div class="balls_diagram__ball with_info_box" data-percent="' + obj.value.toFixed(1).replace(',','.') + '%" data-title="' + obj.label + '" style="margin-left: '+ ball_padding + 'rem;min-width: ' + ball_size + 'rem; min-height: ' + ball_size + 'rem; background: ' + colors[index] + '"></div><div class="balls_diagram__line"></div><div class="balls_diagram__label">' + obj.label + '</div><div class="balls_diagram__data">' + obj.value.toFixed(1).replace(',','.') + '%</div></div>')
		});

		function countRatio(data) {
			var nums = data.map( obj => obj.value);

			return 100 / Math.max.apply(null, nums);
		}
	}
