(function( $ ) {
	...
	$.fn.percentLine = function(line_data) {
		line_data.map( (obj ,index) => {
			$(this).append('<div data-percent="' + obj.value.toFixed(1).replace('.',',') + '%" data-title="' + obj.title + '" class="percent_line_sector" style="width: ' + obj.value + '% ; background:' + colors[index%10] + '"></div>');
		});
	}
